...
int main() {
    ...
    InitWindow(screenWidth, screenHeight, "Pong com Bot - raylib");
    ...
    while (!WindowShouldClose()) {
        ...
        if (ballPosition.y < bot.y + bot.height / 2) bot.y -= 4.0f;
        if (ballPosition.y > bot.y + bot.height / 2) bot.y += 4.0f;

        if (bot.y < 0) bot.y = 0;
        if (bot.y + bot.height > screenHeight) bot.y = screenHeight - bot.height;
        ...
        DrawText("PONG com Bot - raylib", screenWidth/2 - 100, 10, 20, LIGHTGRAY);
